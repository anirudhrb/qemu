...
static uint64_t get_host_property(int mshv_fd, uint32_t prop_code)
{
    int ret;
...
static int create_partition(int mshv_fd, int *vm_fd)
{
    ...
    host_proc_features0 = get_host_property(mshv_fd,
                    HV_PARTITION_PROPERTY_PROCESSOR_FEATURES0);
    host_proc_features1 = get_host_property(mshv_fd,
